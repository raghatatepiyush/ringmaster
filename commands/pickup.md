---
description: Resume Ringmaster work from the .ringmaster/ ledger — reconcile against the repo, then continue the highest-priority pending task.
---

You are resuming a Ringmaster session. Follow the **pickup / resume flow** in
`${CLAUDE_PLUGIN_ROOT}/skills/orchestrator/references/state-and-resume.md` exactly:

1. Read `.ringmaster/state.json` and `.ringmaster/PROGRESS.md`. If neither exists,
   say so plainly and offer to start fresh — do not invent a ledger.
2. Validate: `python "${CLAUDE_PLUGIN_ROOT}/hooks/ledger.py" validate .ringmaster/state.json`
...
5. Select the next task (`python "${CLAUDE_PLUGIN_ROOT}/hooks/ledger.py" next .ringmaster/state.json`,
