...
As you clear each criterion, **record it in the task's `gate`** in the ledger (`correct` from the Test Architect, `secure` from the Security Gate, `clean` from review, `complete` from the acceptance criteria, `documented` from the doc refresh, `explained` from your plain-language summary — plus the conditional `owned` from the ownership review's sign-off when the change calls for one). This isn't bookkeeping for its own sake: a **Stop hook** (`stop_gate.py`) will **block you from ending a turn** while an `in_progress` task's gate is on record as failing — so a task reaches `done` only when it's genuinely A-grade *and*, where required, honestly owned. Check any task deterministically with `python "${CLAUDE_PLUGIN_ROOT}/hooks/ledger.py" gate .conductor/state.json <id>` (PASS, or FAIL with the missing criteria; the variable resolves to Conductor's install directory). A cheaper-model worker that fails the gate escalates to the premium model and re-runs — it never slides through.
